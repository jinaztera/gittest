...
class Turtle(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:  # 아직 주식을 사지 않았을 때
            ...
            if self.datas[0].close > highest:  # 20일 고점 상향 돌파 시 매수
                self.buy(self.datas[0], size=unit)
                self.origin_cost = self.datas[0].close
                self.total_cost = self.broker.getvalue()

        else:
            # 1ATR 만큼 가격이 올랐을 때마다 1단위씩 추가 매수 (5단위까지만 매수 가능)
            if self.datas[0].close > self.origin_cost + self.atr and self.add_unit < 6:
                self.buy(self.datas[0], size=unit)
                self.origin_cost = self.datas[0].close
                self.add_unit += 1
                self.total_cost = self.broker.getvalue()
            ...
            # 총 자산에서 2% 손실 시 손절
            # 10일 저점 하향 돌파 시 매도
            elif self.datas[0].close < lowest or self.broker.getvalue() < self.total_cost * 0.98 or self.datas[
...
# 야후 금융 데이터 불러오기
df = pd.read_excel('Database/TRX.xlsx', index_col="datetime", parse_dates= True)
df = df.rename(columns={"volumns": "volume"})
print(df)
...
# 차트 출력
cerebro.plot()
# plotter=None, numfigs=1, iplot=True, start=None, end=None,
