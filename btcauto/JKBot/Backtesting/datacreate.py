...
for symbol in jongmok:
    ...
    # except:
    btc_ohlcv = binance.fetch_ohlcv(symbol=symbol, timeframe='1d', since=None, limit=2000)
    ...
    df.to_excel('Database2/' + symbol[:-5] + '.xlsx', sheet_name=symbol[:-5])


    # btc_ohlcv = binance.fetch_ohlcv(symbol=symbol, timeframe='1d', since=None, limit=200)
    # df = pd.DataFrame(data=btc_ohlcv, columns=["datetime", "open", "high", "low", "close", "volumns"])
    # df['datetime'] = pd.to_datetime(df['datetime'], unit='ms')
    # # df = df.astype({'datetime': 'str'})
    # df.set_index('datetime', inplace=True)
